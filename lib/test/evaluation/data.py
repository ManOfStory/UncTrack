...
class Sequence:
    ...
    def _construct_init_data(self, init_data):
        if init_data is not None:
            ...
            seqlen = self.ground_truth_rect.shape[0]
            init_data = {}
            for i in range(seqlen):
                init_data.update({i: dict()})     # Assume start from frame 0

                if self.object_ids is not None:
                    init_data[i]['object_ids'] = self.object_ids

                if self.ground_truth_rect is not None:
                    if self.multiobj_mode:
                        assert isinstance(self.ground_truth_rect, (dict, OrderedDict))
                        init_data[i]['bbox'] = OrderedDict({obj_id: list(gt[i,:]) for obj_id, gt in self.ground_truth_rect.items()})
                    else:
                        assert self.object_ids is None or len(self.object_ids) == 1
                        if isinstance(self.ground_truth_rect, (dict, OrderedDict)):
                            init_data[i]['bbox'] = list(self.ground_truth_rect[self.object_ids[0]][i, :])
                        else:
                            init_data[i]['bbox'] = list(self.ground_truth_rect[i,:])

                if self.ground_truth_seg is not None:
                    init_data[i]['mask'] = self.ground_truth_seg[i]
