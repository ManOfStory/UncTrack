...
class Transform:
    ...
    def __call__(self, **inputs):
        ...
        for t in self.transforms:
            ...
            res.append(tuple(out[v] for v in var_names))
        if len(var_names) == 1:
...
class PcaColorJitter(TransformBase):
    ...
    def pca_color_augmentation(self,image_array_input,alpha_std=0.1):
        ...
        rand = np.random.randn(3) * alpha_std
    ...
    def transform_image(self, image, do_colorjitter):
        if do_colorjitter:
            if torch.is_tensor(image):
                raise NotImplementedError('Implement torch variant.')
            img_jittered = self.pca_color_augmentation(image,alpha_std=0.1)
            return img_jittered
...
class Resize(TransformBase):
    ...
    def transform_image(self, image):
        if torch.is_tensor(image):
            raise NotImplementedError('Implement torch variant.')
        img_bgr = cv.resize(image,self.size)
        return img_bgr