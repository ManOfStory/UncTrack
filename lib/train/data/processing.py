...
class MixformerProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            data['template_images'], data['template_anno'], data['template_masks'] = self.transform['joint'](
                image=data['template_images'], bbox=data['template_anno'], mask=data['template_masks'])[-1]
            data['search_images'], data['search_anno'], data['search_masks'] = self.transform['joint'](
                image=data['search_images'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)[-1]
        ...
        for s in ['template', 'search']:
            ...
            res = self.transform[s](
                image=crops, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
            data[s + '_origin_images'] = [res[-2][i][0] for i in range(len(res[-2]))]
            data[s + '_origin_anno'] = [res[-2][i][1] for i in range(len(res[-2]))]

            data[s + '_images'] = [res[-1][i][0] for i in range(len(res[-1]))]
            data[s + '_anno'] = [res[-1][i][1] for i in range(len(res[-1]))]
            data[s + '_att'] = [res[-1][i][2] for i in range(len(res[-1]))]
            data[s + '_masks'] = [res[-1][i][3] for i in range(len(res[-1]))]

            # import matplotlib.pyplot as plt
            # plt.imshow(data[s + '_origin_images'][0].permute(1, 2, 0).detach().cpu().numpy())
            # plt.title('{}'.format(s))
            # plt.axis('off')
            # ax = plt.gca()
            # search_anno = data[s + '_anno'][0]
            # size = data[s + '_origin_images'][0].size(1)
            # rect = plt.Rectangle((int(search_anno[0] * size), int(search_anno[1] * size)),
            #                      int(search_anno[2] * size),
            #                      int(search_anno[3] * size), color="green", fill=False, linewidth=1)
            # ax.add_patch(rect)
            # plt.show()

            #data[s + '_images'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = res[-1]
            #data[s + '_images'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s](
            #    image=crops, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
...
class UncTrackProcessing(BaseProcessing):
    """ The processing class used for training LittleBoy. The images are processed in the following way.
    First, the target bounding box is jittered by adding some noise. Next, a square region (called search region )
    centered at the jittered target center, and of area search_area_factor^2 times the area of the jittered box is
    cropped from the image. The reason for jittering the target box is to avoid learning the bias that the target is
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
        ...
        """
        super().__init__(*args, **kwargs)
        self.transform['memory_template'] = self.transform['template']
        self.transform['memory_search'] = self.transform['search']
        self.search_area_factor = search_area_factor
        self.output_sz = output_sz
        self.center_jitter_factor = center_jitter_factor
        self.scale_jitter_factor = scale_jitter_factor
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['memory_template_images'], data['memory_template_anno'], _ = self.transform['joint'](
                image=data['memory_template_images'], bbox=data['memory_template_anno'], mask= data['memory_template_masks'])[-1]

            data['memory_search_images'], data['memory_search_anno'], _ = self.transform['joint'](
                image=data['memory_search_images'], bbox=data['memory_search_anno'],mask=data['memory_search_masks'])[-1]

            data['search_images'], data['search_anno'], data['search_masks'] = self.transform['joint'](
                image=data['search_images'], bbox=data['search_anno'], mask=data['search_masks'], new_roll=False)[-1]

        for s in ['template', 'search', 'memory_template', 'memory_search']:
            assert self.mode == 'sequence' or len(data[s + '_images']) == 1, \
                "In pair mode, num train/test frames must be 1"
