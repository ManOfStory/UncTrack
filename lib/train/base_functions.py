...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, TNL2k, OnePic
...
def update_settings(settings, cfg):
    ...
    if settings.script_name == 'unctrack_online':
        settings.search_area_factor = {'template': cfg.DATA.TEMPLATE.FACTOR,
                                       'search': cfg.DATA.SEARCH.FACTOR,
                                       'memory_search': cfg.DATA.MEMORY.SEARCH.FACTOR,
                                       'memory_template': cfg.DATA.MEMORY.TEMPLATE.FACTOR}
        settings.output_sz = {'template': cfg.DATA.TEMPLATE.SIZE,
                              'search': cfg.DATA.SEARCH.SIZE,
                              'memory_search': cfg.DATA.MEMORY.SEARCH.SIZE,
                                       'memory_template': cfg.DATA.MEMORY.TEMPLATE.SIZE}
        settings.center_jitter_factor = {'template': cfg.DATA.TEMPLATE.CENTER_JITTER,
                                         'search': cfg.DATA.SEARCH.CENTER_JITTER,
                                         'memory_search': cfg.DATA.MEMORY.SEARCH.CENTER_JITTER,
                                       'memory_template': cfg.DATA.MEMORY.TEMPLATE.CENTER_JITTER}
        settings.scale_jitter_factor = {'template': cfg.DATA.TEMPLATE.SCALE_JITTER,
                                        'search': cfg.DATA.SEARCH.SCALE_JITTER,
                                        'memory_search': cfg.DATA.MEMORY.SEARCH.SCALE_JITTER,
                                       'memory_template': cfg.DATA.MEMORY.TEMPLATE.SCALE_JITTER}
    else:
        settings.search_area_factor = {'template': cfg.DATA.TEMPLATE.FACTOR,
                                       'search': cfg.DATA.SEARCH.FACTOR}
        settings.output_sz = {'template': cfg.DATA.TEMPLATE.SIZE,
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["ONEPIC","LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "COCO17", "VID", "TRACKINGNET", "TNL2k"]
        if name == "LASOT":
        ...
        if name == "ONEPIC":
            datasets.append(OnePic(settings.env.onepic_dir, image_loader=image_loader))
    return datasets
...
def build_dataloaders(cfg, settings):
    ...
    datasets_train = names2datasets(cfg.DATA.TRAIN.DATASETS_NAME, settings, opencv_loader)
    dataset_train = sampler.TrackingSampler(datasets=datasets_train,
    ...
    return loader_train, loader_val, datasets_train

def build_dataloaders_stage2(cfg, settings):
    # Data transform
    transform_joint = tfm.Transform(tfm.ToGrayscale(probability=0.05),
                                    tfm.RandomHorizontalFlip(probability=0.5))
    ...
    data_processing_train = processing.UncTrackProcessing(search_area_factor=search_area_factor,
                                                           output_sz=output_sz,
                                                           center_jitter_factor=settings.center_jitter_factor,
                                                           scale_jitter_factor=settings.scale_jitter_factor,
                                                           mode='sequence',
    ...
    data_processing_val = processing.UncTrackProcessing(search_area_factor=search_area_factor,
                                                         output_sz=output_sz,
                                                         center_jitter_factor=settings.center_jitter_factor,
                                                         scale_jitter_factor=settings.scale_jitter_factor,
                                                         mode='sequence',
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_score:
        ...
        for n, p in net.named_parameters():
            if ("score" not in n) and ("embedder" not in n) and ("conf_conv" not in n):
        ...
        for n, p in net.named_parameters():
            if "score" in n or "backbone" in n:
                p.requires_grad = False
        for n,p in net.named_parameters():
            if "box_head" in n and p.requires_grad:
