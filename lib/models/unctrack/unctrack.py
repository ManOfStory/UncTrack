...
class ConvViT(nn.Module):
    ...
    def forward(self, x_t, x_ot, x_s):
        ...
        x_t = self.pos_drop(x_t) #1 256 32 32

        for blk in self.blocks1:
            x_t = blk(x_t)
        x_t = self.patch_embed2(x_t) #1 384 16 16

        for blk in self.blocks2:
            x_t = blk(x_t)
        x_t = self.patch_embed3(x_t) #1 768 8 8
        ...
        x_t_2d = x_t.transpose(1, 2).reshape(B, C, H_t, W_t) #1 768 8 8
...
class UncTrack(nn.Module):
    def __init__(self, backbone, box_head, head_type="UNC_CORNER",cfg = None):
    ...
    def forward_box_head(self, search):
        ...
        if self.head_type == 'UNC_CORNER':
            ...
            sigma_map = torch.cat([out_dict['sigma_map_tl'], out_dict['sigma_map_br']], dim=1)
            out_dict.update({'sigma_map':sigma_map})
            return out_dict
...
def build_unctrack(cfg, settings = None, train=True) -> UncTrack:
