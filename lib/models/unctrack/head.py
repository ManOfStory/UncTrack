...
from lib.models.unctrack.utils import FrozenBatchNorm2d
...
class Pyramid_Corner_Uncertainty_Predictor(nn.Module):
    """ Corner Predictor module"""
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        super(Pyramid_Corner_Uncertainty_Predictor, self).__init__()
        self.feat_sz = feat_sz
        self.stride = stride
        self.img_sz = self.feat_sz * self.stride
        '''top-left corner'''
        ...
        self.conv6_tl = nn.Conv2d(channel // 8, 2, kernel_size=1)
        ...
        self.adjust5_tl = nn.Sequential(conv(channel // 2, channel // 4, freeze_bn=freeze_bn),
                                        conv(channel // 4, channel // 8, freeze_bn=freeze_bn),
                                        conv(channel // 8, 2, freeze_bn=freeze_bn))
        self.adjust6_tl = nn.Sequential(conv(channel // 4, channel // 8, freeze_bn=freeze_bn),
                                        conv(channel // 8, 2, freeze_bn=freeze_bn))

        self.sigma_tl = nn.Sequential(nn.Flatten(),
                                           nn.Linear(self.feat_sz * self.feat_sz, (self.feat_sz * self.feat_sz) // 2),
                                           nn.BatchNorm1d((self.feat_sz * self.feat_sz) // 2),
                                           nn.ReLU(True),
                                           nn.Linear((self.feat_sz * self.feat_sz) // 2, 2))
        ...
        self.conv6_br = nn.Conv2d(channel // 8, 2, kernel_size=1)
        ...
        self.adjust5_br = nn.Sequential(conv(channel // 2, channel // 4, freeze_bn=freeze_bn),
                                        conv(channel // 4, channel // 8, freeze_bn=freeze_bn),
                                        conv(channel // 8, 2, freeze_bn=freeze_bn))
        self.adjust6_br = nn.Sequential(conv(channel // 4, channel // 8, freeze_bn=freeze_bn),
                                        conv(channel // 8, 2, freeze_bn=freeze_bn))

        self.sigma_br = nn.Sequential(nn.Flatten(),
                                           nn.Linear(self.feat_sz * self.feat_sz, (self.feat_sz * self.feat_sz) // 2),
                                           nn.BatchNorm1d((self.feat_sz * self.feat_sz) // 2),
                                           nn.ReLU(True),
                                           nn.Linear((self.feat_sz * self.feat_sz) // 2, 2))
    ...
    def forward(self, x, return_dist=True, softmax=True):
        """ Forward pass with input x. """
        score_map_tl, score_map_br, sigma_map_tl, sigma_map_br = self.get_score_map(x)
        if return_dist:
            coorx_tl, coory_tl, prob_vec_tl = self.soft_argmax(score_map_tl, return_dist=True, softmax=softmax)
            coorx_br, coory_br, prob_vec_br = self.soft_argmax(score_map_br, return_dist=True, softmax=softmax)
            coord = torch.stack((coorx_tl, coory_tl, coorx_br, coory_br),dim = 1) / self.img_sz
            dict = {
                'coord': coord,
                'prob_vec_tl':prob_vec_tl,#B FeatxFeat
                'prob_vec_br':prob_vec_br,#B FeatxFeat
                'coord_map_x':self.coord_x / (self.feat_sz * self.stride),#B FeatxFeat
                'coord_map_y':self.coord_y / (self.feat_sz * self.stride),#B FeatxFeat
                'search_feat':x,
                'sigma_map_tl':sigma_map_tl,
                'sigma_map_br':sigma_map_br,
            ...
            dict = {
                ...
                'sigma_map_tl': sigma_map_tl,
                'sigma_map_br': sigma_map_br,
                'coord_map_x': self.coord_x,  # B FeatxFeat
                'coord_map_y': self.coord_y,  # B FeatxFeat
            }
        return dict
    def get_score_map(self, x):
        x_init = x
        ...
        score_map_tl = self.conv5_tl(x_tl4) + F.interpolate(self.adjust3_tl(x_tl2), scale_factor=4) + F.interpolate(self.adjust4_tl(x_tl3), scale_factor=2)
        ##回归sigma_lt
        sigma_map_tl = self.conv6_tl(x_tl4) + F.interpolate(self.adjust5_tl(x_tl2), scale_factor=4) + F.interpolate(self.adjust6_tl(x_tl3), scale_factor=2)
        sigma_map_tl = torch.sigmoid(sigma_map_tl)
        #sigma_tl = self.sigma_tl(sigma_map_tl)
        ...
        score_map_br = self.conv5_br(x_br4) + F.interpolate(self.adjust3_br(x_br2), scale_factor=4) + F.interpolate(self.adjust4_br(x_br3), scale_factor=2)
        ##回归sigma_br
        sigma_map_br = self.conv6_br(x_br4) + F.interpolate(self.adjust5_br(x_br2),scale_factor=4) + F.interpolate(self.adjust6_br(x_br3), scale_factor=2)
        #sigma_br = self.sigma_br(sigma_map_br)
        sigma_map_br = torch.sigmoid(sigma_map_br)

        return score_map_tl, score_map_br, sigma_map_tl, sigma_map_br
    ...
    if cfg.MODEL.HEAD_TYPE == "MLP":
        ...
        if cfg.MODEL.HEAD_TYPE == "CORNER":
            ...
            corner_head = Pyramid_Corner_Predictor(inplanes=cfg.MODEL.HIDDEN_DIM, channel=channel,
                                                   feat_sz=feat_sz, stride=stride, freeze_bn=freeze_bn)
        elif cfg.MODEL.HEAD_TYPE == 'UNC_CORNER':
            stride = 4
            feat_sz = int(cfg.DATA.SEARCH.SIZE / stride)
            corner_head = Pyramid_Corner_Uncertainty_Predictor(inplanes=cfg.MODEL.HIDDEN_DIM, channel=channel,
                                                               feat_sz=feat_sz, stride=stride, freeze_bn=freeze_bn)
        else:
            raise ValueError()
        return corner_head
